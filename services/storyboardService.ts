...
初始状态：物体以完整形态存在。
触发动作：物体高速命中或受到能量冲击。
变化过程：原有结构解体、崩散、破裂或消失。
最终状态：新的物质形态覆盖、残留或固定在环境中。


2️⃣  击中、打击 、受力类（非爆炸）
初始状态：人物或物体处于静止或移动状态。
触发动作：外力正面或侧面命中。
变化过程：身体或物体产生位移、形变或震动反馈。
最终状态：人物停在新的位置或维持受力后的姿态。
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { 
          ...
          ------------------------------------------------
          【追加当前批次任务指令】：
          1. **重要**：你现在只负责生成总任务中的【第 ${range} 个】分镜，起始编号为 ${startNo}。
          2. **数量要求**：本批次你必须精准生成 30 个分镜，不得多也不得少。
          3. 请严格基于上述所有【导演演绎区】、【动作与特效拆解规范】以及【viduPrompt 专用规则】进行创作。
          4. 每一个 viduPrompt 必须严格按照你刚才看到的 8 大类范例结构（初始、触发、过程、最终）进行解构，严禁简化。
          5. 为了确保 60 个分镜能够完整传输而不报错，请输出以下简写 JSON 格式（我会由程序自动还原）：
