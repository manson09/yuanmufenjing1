...
你必须根据“视觉奇观价值”而非“文字长度”来分配镜头。

1. **【低信息量区域：叙事脱水（Narrative Dehydration）】**
   - **定义**：一切为了交代逻辑、位置、日常对白的镜头。
   - **执行**：剧本中“李凡来到酒楼”、“众人坐下讨论计划”、“两人边走边聊”这类情节，无论文字多长，**视觉信息量极低**。
   - **指令**：使用【极简剪辑】。合并动作，1-2镜带过所有过场。严禁在没有冲突的对话上拆分镜头。

2. **【高信息量区域：视觉奇观（Visual Spectacle）】**
   - **定义**：主角展示武力（装逼）、反派被打脸瞬间、极端情绪扭曲、环境物理崩坏。
   - **执行**：哪怕剧本只有五个字“李凡弹指一挥”，**视觉信息量也极高**。
   - **指令**：执行【视觉原子化拆解】。必须将这一个动作拆解为 8 个以上的“奇观分镜”：
     - **微观奇观**：手指发力时的指关节摩擦火星、肌肉纤维的震颤。
     - **环境反馈（视觉倍增器）**：脚下的地砖呈环形碎裂、周边的酒杯同时炸裂、空气被压缩产生的白雾。
     - **众生相（情绪倍增器）**：对手瞳孔缩放的瞬间、路人被气浪吹飞的定格、远处飞鸟被惊起的全景。
     - **结算奇观**：多角度重放打击瞬间，残影停留时间拉长。

3. **【镜头分配硬约束】**
   - **过场戏**：镜头平均时长 4s+，镜头极简，快速跳过。
   - **高潮/装逼戏**：镜头平均时长 1.5s-2s，剪辑率极高，视觉冲击点必须像机枪一样密集覆盖。
   - **原则**：你可以压缩前 40 镜的叙事，但必须把最后 20 镜留给高潮部分的“帧级拆解”。
...
const dynamicPrompt = `${STORYBOARD_PROMPT}\n\n${STYLE_PROMPTS[style]}`;
 ...
 let dynamicStrategy = "";
    if (batchIndex === 0) {
      ...
      dynamicStrategy = `
\n【🎬 本阶段执行策略：叙事脱水】
- 当前为剧本开端，请用最少的镜头（1-2镜）交代清楚环境和走位。
- 严禁在平淡对话上浪费镜头，迅速推进到矛盾冲突点。
- 将镜头预算节省下来，留给后续的高潮部分。`;
    } else if (batchIndex === 1) {
      // 第二阶段通常是冲突升级
      dynamicStrategy = `
\n【🎬 本阶段执行策略：节奏拉锯】
- 开始增加情绪特写，通过眼神、肢体细节拉长紧张感。
- 对即将爆发的动作进行“前摇”拆解，为最终高潮蓄势。`;
    } else {
      // 第三阶段通常是爽点爆发
      dynamicStrategy = `
\n【🎬 本阶段执行策略：视觉原子化拆解（极限爽感）】
- 这是最终高潮！请开启“视觉奇观”模式。
- 哪怕剧本只有一句话的动作，也要强制拆解出环境反馈、路人震惊、物理破坏等高密度奇观分镜。
- 每一个动作都要有“起手-冲击-结算-定格”的完整视觉链条。`;
    }
